from pyrtl import PyrtlError, WireVector, Register
...
class RuleTransformer(Transformer):
    ...
    def start(self, rule):
        if rule[0] == "all":
            ...
            key = (stateInBits << self.input_bitwidth) | rule[1]
            return {key : self.states.index(rule[2])}

class FSM:
    def __init__(self, input_bitwidth, states, rulesList, output=False, outputList=[]):
        if output:
            assert len(states) == len(ouputList)
    ...
    def __call__(self):
        if output:
            return self.outputList[self.stateList.index(self.state)]
        else:
            return self.state
